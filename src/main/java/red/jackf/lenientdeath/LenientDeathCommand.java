...
public class LenientDeathCommand {
    ...
    private static int listTagItems(CommandContext<ServerCommandSource> context) {
        var rawArg = context.getArgument("tag", String.class);
        final var arg = rawArg.charAt(0) == '#' ? rawArg.substring(1) : rawArg;
        ...
        if (id == null) {
            ...
            if (tag.isEmpty()) {
                ...
                tag.get().getSecond().stream().forEach(itemEntry -> context.getSource().sendFeedback(() -> Text.literal(" - " + Registries.ITEM.getId(itemEntry.value())), false));
            }
        }
        return 1;
    }
    ...
    private static int addItem(CommandContext<ServerCommandSource> context) {
        final var rawArg = context.getArgument("item", String.class);
        var source = context.getSource();

        var id = Identifier.tryParse(rawArg);
        if (id != null) {
            final var arg = id.getNamespace().equals(Identifier.DEFAULT_NAMESPACE) ? "minecraft:" + id.getPath() : id.toString();
            if (Registries.ITEM.containsId(id)) {
                if (!CONFIG.items.contains(arg)) {
                    CONFIG.items.add(arg);
                    LenientDeath.saveConfig();
                    source.sendFeedback(() -> Text.translatable("lenientdeath.command.success.itemAdded", arg).formatted(SUCCESS), true);
                    return 1;
                } else {
                    source.sendFeedback(() -> Text.translatable("lenientdeath.command.error.itemAlreadyInConfig", arg).formatted(ERROR), false);
                    return 0;
                }
            } else {
                unknownItem(arg, source);
                return 0;
            }
        } else {
            invalidIdentifier(rawArg, source);
            return 0;
        }
    }
